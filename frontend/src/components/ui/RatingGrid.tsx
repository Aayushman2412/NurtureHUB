 ...
 * and single Likert questions (one column, one row).
 */
        ...
        <tbody>
          {rows.map(row => (
            <tr key={row.key} className="border-b border-border last:border-b-0">
              ...
              {columns.map(col => {
                const current = value[row.key]?.[col.key];
                return (
                  <td key={col.key} className="px-3 py-2.5">
                    <div className="flex gap-1.5" role="radiogroup" aria-label={`${row.label} — ${col.label}`}>
                      {Array.from({ length: scale }, (_, i) => i + 1).map(n => {
                        ...
                        return (
                          <button
                            ...
                            onClick={() => onChange(row.key, col.key, n)}
                            className={cn(
